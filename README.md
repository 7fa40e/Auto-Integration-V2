...
画像を送信すると、Gemini APIが「日付・店舗名・金額・品目」を自動で読み取り、指定したGoogleスプレッドシートへ自動記録するLINE Bot。なお、読み取る項目はコード内のプロンプトを書き換えることで任意に指定できる。
...
* **画像解析**: `Gemini 2.5 Flash` を利用し、送られたレシート画像からデータを抽出。
...
* **画像自動仕分け**: 記録した画像は、スプレッドシートごとに自動生成されるGoogleドライブの専用フォルダへ自動保存。
...
2. 「Messaging API設定」タブから **チャネルアクセストークン（長期）** を発行する。
3. 同タブにある **LINEアプリ用QRコード** から、Botを友達追加する。

### 2. Gemini API キーの取得
1. [Google AI Studio](https://aistudio.google.com/) にアクセス。
2. **Get API key** から、新しいAPIキーを発行する。

### 3. 管理帳スプレッドシートの作成
1. Googleスプレッドシートを新規作成し、名前を「マスターシート」などに変更する。名前自体は何でもよい。
2. 1行目に以下のヘッダー（列名）を設定する(一例)。
   * **A列**: `ユーザーID`
   * **B列**: `シートID`
   * **C列**: `シート名`
   * **D列**: `共有キー`
   * **E列**: `フォルダID`
3. このスプレッドシートの **URLからID（`https://docs.google.com/spreadsheets/d/【この部分】/edit`）** をコピーしておく。
